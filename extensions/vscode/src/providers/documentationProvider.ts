...
export class DocumentationProvider implements vscode.TreeDataProvider<DocumentationItem> {
    ...
    async getChildren(element?: DocumentationItem): Promise<DocumentationItem[]> {
        if (!element) {
            ...
            try {
                this.documentation = await this.keykeeperService.getAllDocumentation();
                ...
                return [new DocumentationItem('Failed to load documentation', '', vscode.TreeItemCollapsibleState.None, 'error')];
            }
        } else {
