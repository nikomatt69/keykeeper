...
export class ApiProvidersProvider implements vscode.TreeDataProvider<ApiProviderItem> {
    ...
    async getChildren(element?: ApiProviderItem): Promise<ApiProviderItem[]> {
        if (!element) {
            ...
            try {
                const response = await this.keykeeperService.getAPIProviders();
                this.providers = Array.isArray((response as any)?.data) ? (response as any).data : (Array.isArray(response) ? response : []);
                ...
                return [new ApiProviderItem('Failed to load providers', error instanceof Error ? error.message : '', vscode.TreeItemCollapsibleState.None, 'error')];
            }
        } else {
            ...
            if (element.provider) {
                ...
                if (element.provider.key_patterns && Array.isArray(element.provider.key_patterns) && element.provider.key_patterns.length > 0) {
                    details.push(new ApiProviderItem(
                        'Key Patterns',
