...
export function activate(context: vscode.ExtensionContext) {
    ...
    chatProvider = new ChatProvider(keykeeperService, context.extensionUri);
    ...
    vscode.window.registerTreeDataProvider('keykeeperChat', chatProvider);
    ...
    const commands = [
        ...
        vscode.commands.registerCommand('keykeeper.refreshKeys', () =>
            refreshKeysCommand(apiKeysProvider, projectsProvider, recentProvider, documentationProvider, apiProvidersProvider, mlEngineProvider, chatProvider)
        ),
        ...
        vscode.commands.registerCommand('keykeeper.showDocumentationDetail', (doc) => {
            if (doc && doc.content) {
                vscode.workspace.openTextDocument({
                    content: `# ${doc.title}\n\n${doc.content}`,
                    language: 'markdown'
        ...
        vscode.commands.registerCommand('keykeeper.showProviderDetails', (provider) => {
            if (provider) {
                const details = JSON.stringify(provider, null, 2);
                vscode.workspace.openTextDocument({
                    content: `# ${provider.name || 'API Provider'}\n\n\`\`\`json\n${details}\n\`\`\``,
                    language: 'markdown'
                }).then(document => vscode.window.showTextDocument(document));
            }
        }),
        // Chat commands
        vscode.commands.registerCommand('keykeeper.createNewChat', () =>
            chatProvider.createNewChatSession()
        ),
        vscode.commands.registerCommand('keykeeper.quickChat', () =>
            chatProvider.quickChat()
        ),
        vscode.commands.registerCommand('keykeeper.openChatSession', (session) =>
            chatProvider.openChatSession(session)
        ),
        vscode.commands.registerCommand('keykeeper.deleteChatSession', (session) =>
            chatProvider.deleteChatSession(session)
        ),
        vscode.commands.registerCommand('keykeeper.refreshChat', () =>
